...
レビュー時に「本来の目的から外れた変更」を検出する:
1. PR/コミットメッセージの目的を確認
2. `git diff` の変更がその目的と一致するか検証
3. 無関係な変更（リファクタリング、フォーマット修正、別機能の修正）は別PRに分離を推奨
4. 意図的なスコープ拡大の場合はPR説明に理由を明記させる

## Fix-First Heuristic (from gstack)

問題を報告するだけでなく、修正コードも提示する:
- 各 finding に `Suggested Fix:` を必須で含める
- 機械的に修正可能なもの（import順、フォーマット等）は AUTO-FIX として即座に修正
- 判断が必要なもの（設計変更、API選択等）は ASK として選択肢を提示

## LLM Trust Boundary (from gstack)

AI生成コードは「外部入力」と同等の検証レベルで扱う:
- 存在しないAPIやライブラリの呼び出しがないか確認
- 幻覚による不要なインポート・未使用変数がないか確認
- 過剰な抽象化（必要以上のインターフェース・ラッパー）がないか確認
- AI生成コードと人間のコードの境界で型安全性を検証
