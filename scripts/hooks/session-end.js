...
`;
  ...
  // Always write stable copies so /resume-session can find context
  // even if session-specific .tmp files get cleaned up between sessions
  const currentContent = readFile(sessionFile);
  if (currentContent) {
    // Write .tmp version (primary)
    const latestTmpFile = path.join(sessionsDir, 'latest-session.tmp');
    try {
      writeFile(latestTmpFile, currentContent);
      log(`[SessionEnd] Updated latest-session.tmp`);
    } catch (err) {
      log(`[SessionEnd] Warning: could not update latest-session.tmp: ${err.message}`);
    }
    // Write .md version (backup, persists even if .tmp files are cleaned up)
    const latestMdFile = path.join(sessionsDir, 'latest-session.md');
    try {
      writeFile(latestMdFile, currentContent);
      log(`[SessionEnd] Updated latest-session.md`);
    } catch (err) {
      log(`[SessionEnd] Warning: could not update latest-session.md: ${err.message}`);
    }
  }
