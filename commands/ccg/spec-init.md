...
4. **Validate Multi-Model MCP Tools**
   - Check `codeagent-wrapper` availability: `C:/Users/nagii/.claude/bin/codeagent-wrapper.exe --version`
   - **工作目录**：`{{WORKDIR}}` **必须通过 Bash 执行 `pwd`（Unix）或 `cd`（Windows CMD）获取当前工作目录的绝对路径**，禁止从 `$HOME` 或环境变量推断。如果用户通过 `/add-dir` 添加了多个工作区，先确定任务相关的工作区。
   - Test Codex backend:
     ```bash
     echo "echo test" | C:/Users/nagii/.claude/bin/codeagent-wrapper.exe --backend codex - "{{WORKDIR}}"
     ```
   - Test Gemini backend:
     ```bash
     echo "echo test" | C:/Users/nagii/.claude/bin/codeagent-wrapper.exe --backend gemini --gemini-model gemini-3.1-pro-preview - "{{WORKDIR}}"
     ```
