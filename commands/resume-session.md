...
2. Pick the most recently modified `*-session.tmp` file (excluding `latest-session.tmp`)
3. If no dated session files exist, check for `~/.claude/sessions/latest-session.tmp` as fallback
4. If that also doesn't exist, check for `~/.claude/sessions/latest-session.md` as final fallback
5. If none exist, tell the user:
   ```
